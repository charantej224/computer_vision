...
class IndoorDataSet(Dataset):
    def __init__(self, root_path, classes, train=True, transform_spec=None):
        ...
        self.ids = []
        if train:
            self.csv_path = os.path.join(self.root_path, 'TrainImages_v1.txt')
        else:
            self.csv_path = os.path.join(self.root_path, 'TestImages_v1.txt')
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(os.path.join(self.root_path, 'indoorCVPR_09/Images/', self.ids[item])).convert('RGB')
        if self.transform_spec is not None:
            img = self.transform_spec(img)
        else:
